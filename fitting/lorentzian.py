...
class Lorentzian(FitBase):
        PARAMETERS = ['f0', 'df', 'offset', 'amplitude']

        def __init__(self, polarity=None, **kwargs):
        ...
        def f(f, f0, df, offset, amplitude):
            '''
            offset + amplitude / sqrt( 1 + (2*(f-f0)/df)**2 )
            '''
            return offset+amplitude/numpy.sqrt(1.+(2.*(f-f0)/df)**2)
        ...
        def _guess(self, xs, fs, polarity=None, **kwargs):
            ...
            return OrderedDict(zip(self.PARAMETERS, (f0,df,offset,amplitude))) 
